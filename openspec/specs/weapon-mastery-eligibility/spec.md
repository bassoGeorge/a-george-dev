...
Defines who is entitled to weapon mastery under the 2024 rules — the eligible classes and the "Weapon Master" feat — via the `hasWeaponMastery` predicate and its exported constants, and applies that entitlement to the `AttackList` Mastery column, the all-eight `WeaponMasteries` reference panel, and the serialisable eligibility signal on `getCharacterBrief`.
