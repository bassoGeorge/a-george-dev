...
- **WHEN** neither site nor design-system is in the affected projects list
- **THEN** `production_deploy_matrix` is `[]`

### Requirement: Info job outputs PR deploy matrix
The Info job SHALL compute and output a `pr_deploy_matrix` JSON array using the same structure as `production_deploy_matrix`, except the site entry SHALL be included when either `should_deploy_site` OR `should_run_e2e` is true.

#### Scenario: Only e2e tests changed on PR
...
#### Scenario: Neither site nor e2e changed on PR
- **WHEN** neither `@ageorgedev/ageorgedev` nor `@ageorgedev/ageorgedev-e2e` is in the affected projects
- **THEN** the site entry is absent from `pr_deploy_matrix`
